...
# Parallelisation: pass chunk index and total chunks as args
if len(sys.argv) == 3:
...
r_all = np.arange(0.166, 0.402, 0.002)
r_values = np.array_split(r_all, n_chunks)[chunk]
print(f"Chunk {chunk}/{n_chunks}: processing R = {r_values[0]:.3f} to {r_values[-1]:.3f} ({len(r_values)} values)")
